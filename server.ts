...
function startStraceDiagnostic(ws: WebSocket, payload: any) {
  ...
  // This is the simple command we WANT to work. We will trace it to see why it fails.
  ...
  // `strace` prints all diagnostic data to STDERR. This is our primary log source.
  ...
  diagnosticProcess.on("close", (code) => {
    ...
    broadcast({ type: "DIAGNOSTIC_COMPLETE", message: `Diagnostic finished.` });
  });
}
